...
static const uint8_t RCON[] = { 54, 27, 128, 64, 32, 16, 8, 4, 2, 1};
...
__attribute__((always_inline)) static void aes128_key_schedule_inv_round(uint8_t p_key[16], uint8_t rcon)
{
  uint_fast8_t round;
  uint8_t *p_key_0 = p_key + 12;
  uint8_t *p_key_m1 = p_key_0 - 4;
...
__attribute__((always_inline)) static void KeyExpansion(uint8_t Round10Key[16]) {
  #pragma clang loop vectorize(enable)
  for (uint_fast8_t i=0;i<sizeof(RCON);i++) {
